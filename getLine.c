	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			...
			rm_Comments(*buf);
			bHistory_list(info, *buf, info->histCount++);
			/* if (_strChracter(*buf, ';')) is this a command chain? */
			{
				*len = r;
				info->cmdBuffer = buf;
			}
 ...
 * Return: bytes read
 */
ssize_t get_inputs(CommandInfo *info)
{
 ...
 * Return: r
 */
ssize_t read_buf(CommandInfo *info, char *buf, size_t *i)
{
	ssize_t r = 0;
 ...
 * Return: s
 */
int _getline(CommandInfo *info, char **ptr, size_t *length)
{
